...
The data sets were then combined by matching the user and business id's together, In total, the data consisted of 33 independent variables across 1,398,056 observations. The data was then split into a training and test data set in order to have 10000 observations in the test set which would provide a sufficient test of the model without reducing too much information needed to train it.
Large amounts of missing values were present with the users data but the nature (and potential importance) of this data made it more manageable to impute. When combined with the other fields, there was only user data for approximately 20% of reviews, attributed to a lack of account. As most of the data was count data, and was already skewed towards zero, the lack of account suggests that the user could also be attributed a zero value as any non-zero value would have been a false attribution of praise to the user and due to the fact it would have been applied in bulk, it would have shifted the distribution more than attributing a zero value to everyone. The most difficult data to impute was the user's stars as they were values in the range of 1 to 5 based on the user's average review. In this case the distribution was analysed and the median value was assigned to all the missing values which, although not ideal, was done to ensure the average was not misrepresented. The ideal solution would have been to assign all users without an account, and therefore missing the average stars value, the number of stars they provided for the review in question. Although this could have worked for the training data set, the nature of attempting to predict stars means that it would not work with the test data set as it would constitute information leak and would not be scalable when the model was applied to data to predict missing values of stars.

##Modelling
After plotting the data, it was clear that stars is a categorical variable with five potential categories. However, due to the nature of the categories having an order and that logically a person who voted a one was more likely to vote a two than any other number so they could be modeled using a linear regression model. The only two categorical variables in the cleaned data set was state and city, so they would need to be removed in order to run a regression model, but otherwise variables like business stars and user stars were already showing linear relation to the stars variable in the graphs. After creating a base linear model using all the data, the coefficients show that there are 17 coefficients with statistically significant correlation and one that has perfect collinearity with another independent variable. The linear model has a mean squared error of 1.502983. Due to the stars data being discrete and positive, the model was run with the poisson distribution, but the MSE was identical therefore, as the stars variable isn't technically count data the model will continue with the linear estimation. The results from the linear model show that not all the predictors are actually working to help predict and therefore the model can definitely be improved. By utilising a Least Absolute Shrinkage and Selection Operator (LASSO) model, it's penalisation parameter will reduce variables that are not helpful in predicting stars to zero, and shrink the others, especially useful with 31 variables.
